...
return function (App $app): void {
    ...
    $controller = $container->get(ExpenseScheduleController::class);
    $auth = $container->get(AuthMiddleware::class);
    $managementRoles = [User::ROLE_CEO, User::ROLE_MANAGER];

    $app->group('/v1/expense-schedules', function ($group) use ($controller, $managementRoles) {
        $group->get('', [$controller, 'index']);
        $group->get('/{id}', [$controller, 'show']);

        $group->post('', [$controller, 'create'])->add(new RoleMiddleware($managementRoles));
        $group->post('/process', [$controller, 'process'])->add(new RoleMiddleware($managementRoles));
